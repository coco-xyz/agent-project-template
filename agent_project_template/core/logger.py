...
def get_logging_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        "handlers": handlers,
        "loggers": {
            # Agent Project Template application loggers
...
def get_logger(name: str) -> logging.Logger:
    ...
    # Get logger with agent_project_template prefix if not already present
    if not name.startswith("agent_project_template"):
        name = f"agent_project_template.{name}"
...
def setup_logger(name: Optional[str] = None) -> logging.Logger:
    ...
    if name is None:
        name = "agent_project_template.default"
