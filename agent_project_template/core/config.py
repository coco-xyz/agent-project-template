...
Application settings and environment configuration for Agent Project Template.
"""
...
class Settings(BaseSettings):
    ...
    def validate_log_level(cls, v: str) -> str:
        ...
        if normalized not in allowed_levels:
            raise ValueError(f"log_level must be one of {sorted(allowed_levels)}, got '{v}'")
        return normalized
    
    # API settings
    api__title: str = Field(default="Agent Project Template", description="API title")
    api__description: str = Field(default="AI Agent Application", description="API description")
    ...
    cors__allow_methods: str = Field(default="GET,POST,PUT,DELETE", description="Allowed HTTP methods (comma-separated)")
    ...
    database__pool_size: int = Field(default=10, ge=1, description="Connection pool size")
    database__max_overflow: int = Field(default=20, ge=0, description="Maximum overflow connections")
    database__pool_timeout: int = Field(default=30, ge=0, description="Pool timeout in seconds")
    database__pool_recycle: int = Field(default=3600, ge=0, description="Pool recycle time in seconds")
    ...
    ai__openai_api_key: Optional[SecretStr] = Field(default=None, description="OpenAI API key")
    ai__anthropic_api_key: Optional[SecretStr] = Field(default=None, description="Anthropic API key")
    ai__google_api_key: Optional[SecretStr] = Field(default=None, description="Google API key")
    ai__openrouter_api_key: Optional[SecretStr] = Field(default=None, description="OpenRouter API key")
    
    # Model configurations
    ai__default_model__provider: str = Field(default="openrouter", description="Default model provider")
    ai__default_model__name: str = Field(default="openai/gpt-4o-mini", description="Default model name")
    
    # Business-oriented model configurations (examples)
    ai__demo_agent__provider: str = Field(default="openrouter", description="Provider for demo agent")
    ai__demo_agent__model_name: str = Field(default="openai/gpt-4o-mini", description="Model name for demo agent")
    ...
    ai__fallback__provider: str = Field(default="openai", description="Provider for fallback model")
    ai__fallback__model_name: str = Field(default="gpt-4o-mini", description="Model name for fallback model")

    # Logfire monitoring settings
    logfire__enabled: bool = Field(default=False, description="Enable Logfire monitoring")
    ...
    logfire__token: Optional[SecretStr] = Field(default=None, description="Logfire token")
    ...
    logfire__instrument__pydantic_ai: bool = Field(default=True, description="Enable Logfire pydantic-ai instrumentation")
    logfire__instrument__redis: bool = Field(default=True, description="Enable Logfire Redis instrumentation")
    logfire__instrument__httpx: bool = Field(default=True, description="Enable Logfire HTTPX instrumentation")
    logfire__instrument__fastapi: bool = Field(default=True, description="Enable Logfire FastAPI instrumentation")
    logfire__httpx_capture_all: bool = Field(default=False, description="Capture all HTTPX requests (can be verbose)")

    # Logging file settings (optional)
    log__dir: str = Field(default="logs", description="Directory where log files are stored")
    log__file_path: Optional[str] = Field(default=None, description="Custom log file path; overrides log__dir if set")
    log__file_level: str = Field(default="INFO", description="File handler log level")
    log__file_max_bytes: int = Field(default=10 * 1024 * 1024, gt=0, description="Max size of a log file before rotation")
    log__file_backup_count: int = Field(default=3, ge=0, description="Number of backup log files to keep")

    # File upload settings
    upload_max_size: int = Field(default=10 * 1024 * 1024, gt=0, description="Maximum file upload size in bytes (10MB)")
    upload_allowed_extensions: str = Field(default=".txt,.md,.json", description="Allowed file extensions (comma-separated)")
...
def create_settings() -> Settings:
    ...
    try:
        ...
        if api_keys_count == 0:
            ...
            print("   Set AI__OPENAI_API_KEY, AI__ANTHROPIC_API_KEY, etc. in your .env file")
